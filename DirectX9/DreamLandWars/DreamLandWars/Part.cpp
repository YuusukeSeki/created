...
// コンストラクタ
//-----------------------------------------------------------------------------
Part::Part() : Object(Object::TYPE::TYPE_MODEL)
{
	// メンバ変数初期化
	m_pMesh = nullptr;
	m_pMeshMat = nullptr;
...
// コンストラクタ
//-----------------------------------------------------------------------------
Part::Part(Object::TYPE type) : Object(type)
{
	// メンバ変数初期化
	m_pMesh = nullptr;
	m_pMeshMat = nullptr;
...
// 実体の生成
//-----------------------------------------------------------------------------
Part* Part::Create(D3DXVECTOR3& position, const char* FileName)
{
	Part* pPart = new Part(Object::TYPE::TYPE_MODEL);
	pPart->Init(position, FileName);
...
//	増減処理
// 座標の移動
void Part::MovePosition(D3DXVECTOR3& movePosition)
{
	// 座標の移動
...
// 大きさの増減
void Part::MoveSize(D3DXVECTOR3& moveSize)
{
	// 大きさの増減
...
// 回転率の増減
void Part::MoveRotate(D3DXVECTOR3& moveRotate)
{
	// 回転率の増減
	m_rotate += moveRotate;
...
// 拡縮率の増減
void Part::MoveScale(D3DXVECTOR3& moveScale)
{
	// 拡縮率の増減
	m_scale += moveScale;
...
// 色の増減
void Part::MoveColor(int r, int g, int b, int a)
{
	// 色の増減
...
// 設定処理
// 座標の設定
void Part::SetPosition(D3DXVECTOR3& position)
{
	// 座標の移動
...
// 大きさの設定
void Part::SetSize(D3DXVECTOR3& size)
{
	// 大きさの設定
...
// 回転率の設定
void Part::SetRotate(D3DXVECTOR3& rotate)
{
	// 回転率の増減
	m_rotate = rotate;
...
// 拡縮率の設定
void Part::SetScale(D3DXVECTOR3& scale)
{
	// 拡縮率の設定
	m_scale = scale;
...
// 色の設定
void Part::SetColor(unsigned int rgba)
{
	// 色設定
	m_color.color = rgba;
...
// 対象の方向を向くように設定する
void Part::SetRotateToObj(D3DXVECTOR3& ObjectPostion)
{
...
//	private関数
// 頂点バッファの生成　※未完成
void Part::MakeVertexBuf()
{

}

// 頂点バッファの更新　※未完成
void Part::UpdateVertexBuf()
{
	// 頂点数取得
...
void Part::UpdateWorldMatrix()
{
	...
	if (m_pParent != nullptr) {
		D3DXMatrixMultiply(&m_WorldMatrix, &m_WorldMatrix, &m_pParent->GetWorldMatrix());
	}
...
// モデルデータの読み込み　※未完成
void Part::LoadModel(const char* FileName)
{
...
void Part::LoadModelSizeFromX(const char* FileName)
{
	// 変数の定義
	unsigned int positionNum = 0;
...
// マテリアル情報の読み込み　※未完成
void Part::LoadMaterial()
{

}

// モデルデータの読み込み（directXの便利関数使用）
void Part::LoadMeshModel_DX(const char* modelPass)
{
	// デバイスの取得
	LPDIRECT3DDEVICE9 pDevice = Renderer::GetDevice();
